...
import 'package:nfc/utility/extensions.dart';
...
class _TagInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (Platform.isAndroid) {
      var text =
          '${(NfcA.from(tag)?.identifier ?? NfcB.from(tag)?.identifier ?? NfcF.from(tag)?.identifier ?? NfcV.from(tag)?.identifier ?? Uint8List(0)).toString()}';
      print("WOI : " + text);
      ...
      if (tech is NfcA) {
        tagWidgets.add(FormRow(
          ...
          subtitle: Text('${tech.atqa.toString()}'),
        ));
        tagWidgets.add(FormRow(
          title: Text('NfcA - Sak'),
